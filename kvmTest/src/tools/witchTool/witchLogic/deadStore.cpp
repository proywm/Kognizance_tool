...
DeadStore* DeadStore::ds_pInstance = NULL;

thread_local uint64_t DeadStore::totalDeadBytes_perThread = 0;
thread_local uint64_t DeadStore::totalUsedBytes_perThread = 0;
thread_local uint64_t DeadStore::watchedSampleCount_perThread = 0;

uint64_t DeadStore::totalDeadBytes = 0;
uint64_t DeadStore::totalUsedBytes = 0;
uint64_t DeadStore::watchedSampleCount = 0;
...
DeadStore* DeadStore::Instance()
{
   if (!ds_pInstance)   // Only allow one instance of class to be generated.
      ds_pInstance = new DeadStore;

   return ds_pInstance;
}


//WP_TriggerAction_t DeadStore::Witch_OnWP_Callback(WP_TriggerInfo_t *wpi){
WP_TriggerAction_t DeadStore::Witch_OnWP_Callback(Event *currEvent, Event *sampleEvent, WP_TriggerInfo_t *wpt){
...
#if defined WITCHTOOL_DEADSTORE_INST
...
//		std::cout << TOOLMSG_BEGIN << std::hex << "sh addr2line.sh 0x"<< sampleEvent->fixedIP << " 0x" << currEvent->fixedIP << std::dec << std::endl ;
		...
		if (currEvent->accessType == LOAD || currEvent->accessType == LOAD_AND_STORE) {
                	totalUsedBytes += inc;
	        ...
	        } else if (STORE == currEvent->accessType){
...
void DeadStore::printContextTree(int bestNContextNode)
{
        contextTree_mtx.lock();
#if defined WITCHTOOL_DEADSTORE_INST
	MyLogger *logger = MyLogger::Instance();
        calculateForAllContext(&contextTreeRoot, 0);
	unsigned int totalBytes = totalDeadBytes + totalUsedBytes;
