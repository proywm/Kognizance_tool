...
//		printf("Offset %d Size %d\n",offset,size);
		...
		case PERF_RECORD_SAMPLE:
			...
			if (sample_type & PERF_SAMPLE_TIME) {
				...
				eventPtr->event_time = time;
				offset+=8;
			}
			if (sample_type & PERF_SAMPLE_ADDR) {
				unsigned long long addr;
				memcpy(&addr,&data[offset],sizeof(unsigned long long));
				if (!quiet) printf("\tPERF_SAMPLE_ADDR, addr: %llx\n",addr);
				eventPtr->h_data_addr = addr;
				offset+=8;
			}
...
#if 0
			if (sample_type & PERF_SAMPLE_INDEX) {
                                uint64_t sample_index;

                                memcpy(&sample_index,&data[offset],sizeof(uint64_t));
                                if (!quiet) printf("\tPERF_SAMPLE_INDEX, sample_index: %lld\n",sample_index);
				eventPtr->sample_index = sample_index;
                                offset+=8;
